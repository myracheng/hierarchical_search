# pronear/pro_near/scores_436774.npy
...
count = 0
save_dir = os.path.join('trees_holes','fromactual',timestamp)
...
for filename in glob.iglob(root_dir + 'mars**4**/full*.p', recursive=True): #1.p
    print(filename)
    if "neursym" in filename:
        continue
    full_folder = os.path.dirname(filename)
    log_file = os.path.join(full_folder, 'log.txt')
    parameter_file = os.path.join(full_folder, 'parameters_0.txt')

    # base_program_name: program_96
    # try:
    ### GET PROGRAM
    f = open(parameter_file, "r")
    lines_list = f.readlines()
    lines = "".join(lines_list)
    num_program = int(re.findall("base_program_name: program_\d+",lines)[-1].split('_')[-1])
    print(num_program)
    if num_program in found_programs:
        continue
    else:
        found_programs[num_program] = True
