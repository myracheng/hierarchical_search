...
def my_mean(x, axis = 0):
    return np.average(x, weights=np.ones_like(x) / len(x), axis=axis)
...
def normalize_data(train_data, valid_data, test_data):
    ...
    train_data_reshape = np.reshape(train_data, (-1, input_dim)).astype('float64')
    test_data_reshape = np.reshape(test_data, (-1, input_dim)).astype('float64')
    features_mean = np.mean(train_data_reshape, axis=0)
    # features_mean = my_mean(train_data_reshape, axis=0)
    ...
    for seq in train_data_reshape:
        seq_sum = np.sum(seq)
        if np.isnan(seq_sum) or np.isinf(seq_sum):
            print(seq)
    for seq in test_data_reshape:
        seq_sum = np.sum(seq)
        if np.isnan(seq_sum) or np.isinf(seq_sum):
            print(seq)
    ...
    if valid_data is not None:
        valid_data_reshape = np.reshape(valid_data, (-1, input_dim)).astype('float64')
        valid_data_reshape = (valid_data_reshape - features_mean) / features_std
        for seq in valid_data_reshape:
            seq_sum = np.sum(seq)
            if np.isnan(seq_sum) or np.isinf(seq_sum):
                print(seq)
