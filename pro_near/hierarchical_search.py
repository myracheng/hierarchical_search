...
--normalize --max_depth 3 --max_num_units 4 --min_num_units 4 --max_num_children 6 --learning_rate 0.001 --neural_epochs 4 --symbolic_epochs 4 \
--class_weights "0.3,0.7" --base_program_name data/7keypoints/astar_1 --hole_node_ind -1 --batch_size 256
...
python3.8 hierarchical_search.py --algorithm ballscreen --exp_name bball --trial 1 \
...
import time
from algorithms import ASTAR_NEAR, IDDFS_NEAR, MC_SAMPLING, ENUMERATION, GENETIC, RNN_BASELINE,BALLSCREEN
...
class Subtree_search():
    ...
    def __init__(self, **kwargs):
        ...
        if self.exp_name == 'crim13' or 'bball' in self.exp_name:
            ...
            # print(self.batched_trainset.shape)
    ...
    def gnn_h(self):
        ...
        for hole_node_ind in range(1,len(l)):
            ...
            data = new_prog.submodules  
            G = nx.Graph()
            self.save_to_tree(data, G)
            score = eval_single_graph(G)
            log_and_print(str(score))
            subprogram_str = print_program(hole_node[0])
    ...
    def run_bball(self):
        ...
        # Initialize program graph starting from trained NN
        program_graph = ProgramGraph(DSL_DICT, CUSTOM_EDGE_COSTS, "list", "list", 51, 2,
            self.max_num_units, self.min_num_units, self.max_num_children, self.max_depth, self.penalty, ite_beta=self.ite_beta)

        # Initialize algorithm
        algorithm = BALLSCREEN()
