...
def get_graph_stats(graph_obj_handle, prop='degrees'):
    ...
    if 'degrees' in prop:
        stat_dict['degrees'] = list(set([d for core_res in res for d in core_res['degrees']]))
    if 'edge_labels' in prop:
...
def get_inaccurate(output,target):
    return false
    # print(output.cpu())
    # print(target.cpu())
    # return np.array(output.cpu()) != np.array(target.cpu())
