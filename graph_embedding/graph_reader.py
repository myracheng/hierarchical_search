...
def divide_datasets_regression(graphs, scores,batch_size):
    tr_idx = []
    va_idx = []
    te_idx = []
    ...
    if len(tr_idx) % batch_size == 1:
        tr_idx = tr_idx[:-1]
    if len(va_idx) % batch_size == 1:
        va_idx = va_idx[:-1]
    if len(te_idx) % batch_size == 1:
        te_idx = te_idx[:-1]
    ...
    train_scores = [scores[i] for i in tr_idx]
    valid_scores = [scores[i] for i in va_idx]
    test_scores = [scores[i] for i in te_idx]
    
    return train_graphs, train_scores, valid_graphs, valid_scores, test_graphs, test_scores

def divide_datasets_leave_one_out(graphs, classes,batch_size):
    #last class
    # test_graphs = graphs[-20:]
    ...
    #middle class
    # 31:(31+88)
    test_graphs = graphs[31:(31+88)]
    test_classes = classes[31:(31+88)]

    graphs = [g for g in graphs if g not in test_graphs]
    classes = [c for g,c in zip(graphs,classes) if g not in test_graphs]
    print(len(graphs))
    print(len(classes))
    uc = list(set(classes))
    tr_idx = []
...
def divide_datasets(graphs,classes):
    ...
    # tr_idx += sorted(np.random.choice(idx, int(0.8*len(idx)), replace=False))
    # va_idx += sorted(np.random.choice([x for x in idx if x not in tr_idx], int(0.2*len(idx)), replace=False))
    # te_idx = list(range(len(scores)-21,len(scores)))
